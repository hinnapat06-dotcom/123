...
const GoldCoinIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="9.5" fill="url(#goldGradient1)" stroke="#d97706" strokeWidth="1.2" />
    <circle cx="12" cy="12" r="7" stroke="#fef08a" strokeWidth="1" strokeDasharray="2.5 1.5" />
    <circle cx="12" cy="12" r="5" fill="url(#goldGradient2)" opacity="0.9" />
    <text x="12" y="15.2" fontSize="9.5" fontWeight="900" textAnchor="middle" fill="#78350f" fontFamily="sans-serif">฿</text>
    <defs>
      <linearGradient id="goldGradient1" x1="4" y1="3" x2="20" y2="21" gradientUnits="userSpaceOnUse">
        <stop stopColor="#fef08a" />
        <stop offset="0.45" stopColor="#f59e0b" />
        <stop offset="1" stopColor="#b45309" />
      </linearGradient>
      <linearGradient id="goldGradient2" x1="7" y1="7" x2="17" y2="17" gradientUnits="userSpaceOnUse">
...
export default function App() {
  ...
  const [userRole, setUserRole] = useState<'staff' | 'public' | 'vhv' | null>(() => {
  ...
  // Super Admin Emails (ผู้ดูแลระบบที่มีสิทธิ์แก้ไขทุกอย่าง)
  const SUPER_ADMIN_EMAILS = useMemo(() => [
    'chinnapat37428@gmail.com',
    'nongkhae01711@gmail.com',
    'usajityuti@gmail.com'
  ], []);
  ...
  const [authorizedEmails, setAuthorizedEmails] = useState<string[]>(() => {
    try {
      ...
      if (saved) {
        ...
        return Array.from(new Set([...SUPER_ADMIN_EMAILS, ...parsed]));
      }
    } catch (e) {}
    // Seed with super admins and mock staff emails
    return [
      ...SUPER_ADMIN_EMAILS,
      'somchai.staff@phaitam.go.th',
      'somsri.staff@phaitam.go.th',
      'somchai@gmail.com'
    ];
  });
  ...
  const resolveAndSetUserRole = (email: string | null, list: string[] = authorizedEmails) => {
    ...
    if (SUPER_ADMIN_EMAILS.some(a => a.toLowerCase() === normalizedEmail)) return 'staff';
    const isApproved = list.some(e => e.trim().toLowerCase() === normalizedEmail);
    return isApproved ? 'staff' : 'public';
  };

  // Helper to check if current user is VHV (อสม.)
  const isVhvUser = useMemo(() => {
    if (userRole === 'vhv') return true;
    if (user?.email && user.email.includes('phaitam-vhv.go.th')) return true;
    if (user?.displayName && user.displayName.toLowerCase().includes('อสม')) return true;
    return false;
  }, [userRole, user]);

  // User profile modal & custom profile state
  ...
  const [customProfile, setCustomProfile] = useState<{
    ...
    try {
      const saved = localStorage.getItem('phaitam_user_custom_profile_v1');
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    const savedUser = (() => {
      try { return JSON.parse(localStorage.getItem('stitchsync_user') || 'null'); } catch { return null; }
    })();
    return {
      displayName: savedUser?.displayName || (isVhvUser ? 'อสม. สมบูรณ์ สุขใจ' : 'เจ้าหน้าที่ สสอ.วิหารแดง'),
      email: savedUser?.email || (isVhvUser ? 'somboon.vhv@gmail.com' : 'hinnapat06@gmail.com'),
      phone: '081-987-6543',
      photoURL: savedUser?.photoURL || 'https://images.unsplash.com/photo-1622253692010-333f2da6031d?auto=format&fit=crop&w=256&q=80',
      roleName: userRole === 'staff' ? 'เจ้าหน้าที่สาธารณสุขวิหารแดง' : isVhvUser ? 'อสม. ประจำหมู่บ้าน' : 'ผู้ใช้งานระบบ',
      workplace: 'โรงพยาบาลส่งเสริมสุขภาพตำบลไผ่ต่ำ อ.วิหารแดง จ.สระบุรี',
      moo: 'หมู่ 1 ตำบลไผ่ต่ำ',
      badgeId: 'VHV-SRB-10824'
    };
  });

  // Sync customProfile when user changes
  useEffect(() => {
    if (user) {
  ...
  // Handle uploading profile photo from file input
  const handleProfileImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (file.size > 8 * 1024 * 1024) {
      alert('⚠️ ขนาดไฟล์รูปภาพใหญ่เกินไป (กรุณาเลือกไฟล์ขนาดไม่เกิน 8MB)');
      return;
    }
    ...
    reader.onloadend = () => {
      ...
      setCustomProfile(prev => {
        const updated = { ...prev, photoURL: base64Url };
        try {
          localStorage.setItem('phaitam_user_custom_profile_v1', JSON.stringify(updated));
        } catch (err) {}
        return updated;
      });

      if (user) {
        const updatedUser = { ...user, photoURL: base64Url };
        setUser(updatedUser);
        try {
          localStorage.setItem('stitchsync_user', JSON.stringify(updatedUser));
        } catch (e) {}
      }

      if (typeof addLog === 'function') {
        addLog('User Profile', 'อัปโหลดภาพโปรไฟล์ใหม่สำเร็จ', 'success');
      }
    };
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    e.preventDefault();
    try {
      localStorage.setItem('phaitam_user_custom_profile_v1', JSON.stringify(customProfile));
      ...
      if (typeof addLog === 'function') {
        addLog('User Profile', `อัปเดตข้อมูลส่วนตัวของ ${customProfile.displayName} เรียบร้อยแล้ว`, 'success');
      }
      setIsUserProfileModalOpen(false);
      alert('✅ บันทึกข้อมูลส่วนตัวและรูปโปรไฟล์เรียบร้อยแล้ว');
    } catch (e: any) {
      console.error(e);
  ...
  // Registered VHVs/Volunteers state (พร้อมเลขบัตรประชาชน 13 หลัก)
  ...
  // User Access Logs state (ประวัติการเข้าใช้งานระบบล่าสุด)
  const [userAccessLogs, setUserAccessLogs] = useState<UserAccessLog[]>(() => {
    try {
      const saved = localStorage.getItem('stitchsync_user_access_logs_v2');
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    return [
      {
        id: 'SESS-SEED-1',
        time: '21 ก.ค. 2569 22:45:10',
        userName: 'อสม. รัตนาภรณ์ รักษ์ดี',
        email: 'somchai.vhv@gmail.com',
        role: 'อสม. ประจำหมู่ 1',
        ip: '110.168.23.105',
        location: '📍 อ.วิหารแดง จ.สระบุรี (14.3185, 100.9820)',
        device: 'Apple Safari / Chrome (iPhone Mobile)',
        status: 'สำเร็จ',
        isMobile: true
      },
      {
        id: 'SESS-SEED-2',
        time: '21 ก.ค. 2569 21:12:04',
        userName: 'อสม. สมพงษ์ ใจเย็น',
        email: 'sompong.vhv@gmail.com',
        role: 'อสม. ประจำหมู่ 3',
        ip: '49.228.192.14',
        location: '📍 ต.หนองหมู อ.วิหารแดง จ.สระบุรี (14.3105, 100.9521)',
        device: 'Microsoft Edge (Windows 11)',
        status: 'สำเร็จ',
        isMobile: false
      },
      {
        id: 'SESS-SEED-3',
        time: '21 ก.ค. 2569 19:30:22',
        userName: 'คุณมะลิ (ผู้ดูแล Caregiver)',
        email: 'cg.mali@gmail.com',
        role: 'ผู้ดูแลคนไข้',
        ip: '171.96.185.88',
        location: '📍 อ.เมืองสระบุรี จ.สระบุรี (14.5289, 100.9108)',
        device: 'Google Chrome (Android Mobile)',
        status: 'สำเร็จ',
        isMobile: true
      }
    ];
  ...
  // Auto-detect mobile / desktop access session and log automatically
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const ua = navigator.userAgent || '';
    const isLineApp = /Line/i.test(ua);
    const isMobileDevice = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Line/i.test(ua) || (window.innerWidth <= 768);

    let deviceName = 'Google Chrome (Windows / PC)';
    if (isLineApp) {
      if (/iPhone|iPad|iPod/i.test(ua)) deviceName = 'LINE App (iOS In-App Browser)';
      else if (/Android/i.test(ua)) deviceName = 'LINE App (Android In-App Browser)';
      else deviceName = 'LINE App (In-App Browser Mobile)';
    } else if (/iPhone/i.test(ua)) {
      deviceName = 'Apple Safari / Chrome (iPhone Mobile)';
    } else if (/iPad/i.test(ua)) {
      deviceName = 'Apple Safari / Chrome (iPad)';
    } else if (/Android/i.test(ua)) {
      deviceName = 'Google Chrome (Android Mobile)';
    } else if (isMobileDevice) {
      deviceName = 'Mobile Browser (Smartphone)';
    } else if (/Macintosh/i.test(ua)) {
      deviceName = 'Google Chrome (Mac Desktop)';
    } else if (/Linux/i.test(ua)) {
      deviceName = 'Google Chrome (Linux Desktop)';
    }

    const now = new Date();
    const timeStr = now.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' (วันนี้)';
    const name = user?.displayName || (isVhvUser ? 'อสม. สมบูรณ์ สุขใจ' : 'เจ้าหน้าที่ สสอ.วิหารแดง');
    const userEmail = user?.email || (isVhvUser ? 'somboon.vhv@gmail.com' : 'hinnapat06@gmail.com');
    const roleName = userRole === 'staff' ? 'เจ้าหน้าที่ (Staff)' : isVhvUser ? 'อสม. (VHV)' : 'บุคคลทั่วไป (Guest)';

    const currentLog: UserAccessLog = {
      id: 'SESS-' + Date.now(),
      time: timeStr,
      userName: name,
      email: userEmail,
      role: roleName,
      ip: '182.52.231.' + (Math.floor(Math.random() * 100) + 10),
      location: '📍 ต.ไผ่ต่ำ อ.วิหารแดง จ.สระบุรี (14.3218, 100.9842)',
      device: deviceName,
      status: 'ONLINE',
    ...
    setUserAccessLogs(prev => {
      // Mark all previous active logs as 'สำเร็จ'
      const updated = prev.map(l => ({ ...l, status: 'สำเร็จ' as const }));
      // If top item is identical name, device, and time, don't re-append
      if (updated.length > 0 && updated[0].userName === name && updated[0].device === deviceName && updated[0].time === timeStr) {
        return prev;
      }
      const combined = [currentLog, ...updated].slice(0, 50);
      try {
        localStorage.setItem('stitchsync_user_access_logs_v2', JSON.stringify(combined));
      } catch (e) {}
      return combined;
    });
  }, [user, userRole, isVhvUser]);

  // Local sync logging for the Network Logs tab with User ID, IP Address, Location, and Time
  ...
  const addLog = (
    ...
    const currentUserId = customUserId || (user?.email ? user.email : user?.displayName ? user.displayName : 'GUEST-1824');
    const ip = customIp || '182.52.231.42';
    const loc = customLoc || 'ต.ไผ่ต่ำ อ.วิหารแดง จ.สระบุรี (14.3218, 100.9842)';
  ...
  useEffect(() => {
    ...
    if (savedRole && (savedRole === 'public' || savedToken === 'mock-staff-token' || savedToken === 'vhv-no-pass-token' || savedToken === 'staff-gmail-token')) {
      ...
      const resolvedRole = resolveAndSetUserRole(parsedUser.email);
      const effectiveRole = resolvedRole === 'staff' ? 'staff' : savedRole;
      setUser(parsedUser);
      setToken(savedToken);
      setUserRole(effectiveRole as any);
      setNeedsAuth(false);
      setLoading(false);
      addLog('Auth', `กู้คืนระบบแบบรวดเร็วสำเร็จ: [${effectiveRole === 'staff' ? 'เจ้าหน้าที่/ผู้ดูแลระบบ' : 'บุคคลทั่วไป'}] คุณ ${parsedUser.displayName}`, 'success');
  ...
  const handleVhvLogin = (e?: React.FormEvent, selectedInput?: string) => {
    ...
    setTimeout(() => {
      const unifiedUser = {
        ...
        cid: resolvedCid,
        email: `${resolvedName.replace(/[^\u0E00-\u0E7Fa-zA-Z0-9]/g, '.').replace(/\.+/g, '.')}@phaitam-vhv.go.th`,
        photoURL: '',
        isVhv: isVhvRole,
      ...
      setCustomProfile(prev => ({
        ...
        badgeId: resolvedCid ? `CID-${resolvedCid.replace(/\D/g, '').slice(-6)}` : 'VHV-SRB-10824'
      }));

      localStorage.setItem('stitchsync_role', 'vhv');
      localStorage.setItem('stitchsync_user', JSON.stringify(unifiedUser));
      localStorage.setItem('stitchsync_token', 'vhv-no-pass-token');
      
      setNeedsAuth(false);
      setLoading(false);
      addLog('Auth', `เข้าใช้งานบัญชีรวมสำเร็จ: คุณ ${resolvedName} [สิทธิ์: ${roleDisplay}]`, 'success');
    ...
    try {
      ...
      if (token && token !== 'mock-staff-token') {
        try {
          ...
          console.warn('Google API fetch failed, attempting public CSV fallback:', apiErr?.message);
          addLog('Sheets Loader', `Google API ให้ข้อผิดพลาด: ${apiErr?.message || 'ไม่มีสิทธิ์เข้าถึง'} -> กำลังลองดึงแบบ Public Export...`, 'pending');
        }
      }
      ...
      if (!fetchedSuccessfully) {
        ...
        if (!response.ok) {
          throw new Error('ไม่สามารถเข้าถึงแผ่นงานได้ กรุณาตั้งค่าแชร์แผ่นงานเป็น "ทุกคนที่มีลิงก์มีสิทธิ์อ่าน" (Anyone with link can view) หรือตรวจสอบลิงก์ Google Sheets อีกครั้ง');
        }
        ...
        if (rows.length === 0) {
          throw new Error('ไม่พบแถวข้อมูลจากการดึงข้อมูลแผ่นงาน');
        }
        ...
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-50 md:hidden flex">
            ...
            >
              ...
              {/* User Profile Quick Card in Drawer */}
              <div 
                onClick={() => {
                  setIsMobileMenuOpen(false);
                  setIsUserProfileModalOpen(true);
                }}
                className="p-3 mx-4 mt-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/80 rounded-2xl flex items-center gap-3 cursor-pointer hover:border-blue-400 transition-all shadow-2xs"
                title="คลิกเพื่อดูและแก้ไขข้อมูลส่วนตัว"
              >
                <div className="w-10 h-10 rounded-full border-2 border-blue-300 overflow-hidden bg-white shrink-0 shadow-2xs relative">
                  <img
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-extrabold text-xs text-slate-800 truncate">{customProfile.displayName}</div>
                  <div className="text-[10px] text-blue-700 font-semibold truncate">{customProfile.roleName}</div>
                </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div 
                ...
                title="คลิกเพื่อดูและแก้ไขข้อมูลส่วนตัว / อัปโหลดรูปภาพ"
              >
                <div className="text-xs sm:text-sm font-bold text-slate-800 flex items-center gap-1.5 leading-tight group-hover:text-blue-600 transition-colors">
                  <span>{customProfile.displayName || user?.displayName || 'Chaiwat Somchai'}</span>
                </div>
                <div className="text-[10px] text-slate-500">{customProfile.email || user?.email || 'chaiwat.s@gmail.com'}</div>
              </div>

              <div 
                onClick={() => setIsUserProfileModalOpen(true)}
                className="relative w-8 h-8 sm:w-10 sm:h-10 bg-slate-100 rounded-full border-2 border-blue-200 shadow-sm flex items-center justify-center text-slate-600 font-bold overflow-hidden shrink-0 cursor-pointer hover:border-blue-500 hover:scale-105 transition-all group"
                title="คลิกเพื่อดูและแก้ไขข้อมูลส่วนตัว / อัปโหลดรูปภาพ"
              >
                <img 
                  src={customProfile.photoURL || user?.photoURL || 'https://images.unsplash.com/photo-1622253692010-333f2da6031d?auto=format&fit=crop&w=256&q=80'} 
        ...
        <div className="flex-1 w-full h-full relative z-10 overflow-hidden bg-slate-50">
          <AnimatePresence mode="wait">
            ...
            {currentTab === 'dashboard' && (
              ...
              >
                {/* Dashboard Map View (Original Google My Maps Mind Map) */}
                      ...
                      <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
                        ...
                        <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar touch-scroll">
                          {filteredDbPatients.length === 0 ? (
                            <div className="p-6 sm:p-12 text-center text-slate-500 space-y-3 bg-rose-50/30 rounded-2xl border border-dashed border-rose-200 m-4">
                              <div className="w-12 h-12 rounded-full bg-rose-100 flex items-center justify-center mx-auto text-rose-600">
                                <Heart className="w-6 h-6" />
                              </div>
                              <div>
                                <p className="text-sm font-bold text-slate-800">ไม่พบรายชื่อผู้ป่วยในมุมมองปัจจุบัน</p>
                                <p className="text-xs text-slate-500 mt-0.5">
                                  {ptSearchQuery || ptMooFilter !== 'ทั้งหมด'
                                    ? `เงื่อนไขการค้นหา "${ptSearchQuery}" (หมู่บ้าน: ${ptMooFilter}) ไม่พบรายการ`
                                    : 'ขณะนี้ไม่มีข้อมูลผู้ป่วยในอุปกรณ์เครื่องนี้'}
                                </p>
                              </div>
                              <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
                                {(ptSearchQuery || ptMooFilter !== 'ทั้งหมด') && (
                                  <button
                                    type="button"
                                    onClick={() => { setPtSearchQuery(''); setPtMooFilter('ทั้งหมด'); }}
                                    className="px-3.5 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-bold shadow-2xs cursor-pointer"
                                  >
                                    🔄 ล้างตัวกรองการค้นหา
                                  </button>
                                )}
                                <button
                                  ...
                                  onClick={() => {
                                    setPatients(SEED_PATIENTS);
                                    setPtSearchQuery('');
                                    setPtMooFilter('ทั้งหมด');
                                    localStorage.setItem('stitchsync_patients', JSON.stringify(SEED_PATIENTS));
                                    addLog('Local DB', 'โหลดรายชื่อผู้ป่วยตั้งต้น 69 รายสำเร็จ', 'success');
                                    alert('✅ โหลดรายชื่อผู้ป่วยตั้งต้น 69 ราย (ตำบลไผ่ต่ำ) เข้าสู่ระบบเรียบร้อยแล้ว');
                                  }}
                                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold shadow-xs cursor-pointer transition-all active:scale-95 flex items-center gap-1.5"
                                >
                                  <Download className="w-3.5 h-3.5" />
                                  <span>📥 ดึงข้อมูลรายชื่อผู้ป่วยตั้งต้น 69 ราย (ตำบลไผ่ต่ำ)</span>
                                </button>
                              </div>
                            </div>
                          ) : (
                                ...
                                {filteredDbPatients.map((item) => (
                                  <div key={item.id} className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-2">
                                    ...
                                    <div 
                                      ...
                                      className="font-bold text-rose-600 text-sm hover:underline cursor-pointer"
                                    >
                                      {item.name}
                                    </div>
                                    <div className="text-xs text-slate-600 space-y-1">
                                      <p className="flex items-center justify-between">
                                        <span className="text-slate-400 font-semibold">หมู่บ้าน:</span>
                                        <span className="font-bold text-slate-700">{item.moo || 'หมู่ 1'}</span>
                                      </p>
                                      <p className="flex items-center justify-between">
                  ...
                  <div className="flex-1 overflow-hidden flex flex-col h-full bg-slate-50">
                    ...
                    <div className="flex-1 overflow-y-auto lg:overflow-hidden flex flex-col lg:flex-row p-3 sm:p-6 gap-4 sm:gap-6 touch-scroll">
                      ...
                      <div className="w-full lg:w-[380px] bg-white rounded-2xl border border-slate-200 p-5 shadow-sm shrink-0 overflow-y-auto custom-scrollbar flex flex-col space-y-4">
                        <div>
                          <div className="flex items-center justify-between">
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider font-mono">ลงทะเบียน คลังทุนหนุนใจ</h3>
                            {userRole === 'staff' ? (
                  ...
                  <div className="flex-1 overflow-y-auto pr-2 space-y-1.5 custom-scrollbar font-mono text-[11px] leading-relaxed">
                    {networkLogs.length === 0 ? (
                      ...
                      networkLogs
                        ...
                        .map((log, index) => (
                          <div key={index} className="p-2 bg-slate-900/80 hover:bg-slate-900 rounded-lg border border-slate-800/80 transition-colors space-y-1">
                            <div className="flex flex-wrap items-center gap-2 text-[10px]">
                              <span className="text-slate-500 font-bold shrink-0">#{networkLogs.length - index}</span>
                              <span className="bg-amber-950/80 text-amber-300 font-bold px-1.5 py-0.5 rounded border border-amber-800/60">
                                ⏱️ {log.time}
                              </span>
                              <span className="bg-cyan-950/80 text-cyan-300 font-bold px-1.5 py-0.5 rounded border border-cyan-800/60">
                                👤 ID: {log.userId}
                              </span>
                              <span className="bg-blue-950/80 text-blue-300 font-bold px-1.5 py-0.5 rounded border border-blue-800/60">
                                🌐 IP: {log.ipAddress}
                              </span>
                              <span className="bg-emerald-950/80 text-emerald-300 font-bold px-1.5 py-0.5 rounded border border-emerald-800/60">
                                📍 ที่ตั้ง: {log.location}
                              </span>
                            ...
                            <div className={`text-[11px] pl-1 break-words font-sans ${
                    ...
                    <div className="md:col-span-2 space-y-2">
                      ...
                      <div className="border border-slate-200 rounded-lg overflow-hidden divide-y divide-slate-100 max-h-[220px] overflow-y-auto bg-slate-50">
                        {authorizedEmails.map((email) => {
                          ...
                          const isSuperAdmin = SUPER_ADMIN_EMAILS.some(a => a.toLowerCase() === email.toLowerCase());
                          return (
                            <div key={email} className="flex items-center justify-between p-2.5 bg-white text-xs text-slate-700">
                              <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                <span className="font-mono text-slate-700 font-medium">{email}</span>
                                {isSuperAdmin && (
                                  <span className="bg-purple-50 border border-purple-200 text-purple-700 px-1.5 py-0.5 rounded text-[9px] font-bold">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการยกเลิกอนุมัติสิทธิ์สำหรับอีเมล ${email}?`)) {
                                    if (SUPER_ADMIN_EMAILS.some(a => a.toLowerCase() === email.toLowerCase())) {
                                      alert('⚠️ ไม่สามารถยกเลิกสิทธิ์อีเมลผู้ดูแลระบบหลัก (Super Admin) ได้');
                                      return;
                                    }
                                    const newList = authorizedEmails.filter(e => e !== email || SUPER_ADMIN_EMAILS.some(a => a.toLowerCase() === e.toLowerCase()));
                                    setAuthorizedEmails(newList);
                              ...
                              >
                                {isSuperAdmin ? 'สิทธิ์ผู้ดูแลหลัก' : isCurrentUser ? 'ไม่สามารถเพิกถอนสิทธิ์ตนเองได้' : 'เพิกถอนสิทธิ์'}
                              </button>
                            </div>
                          );
                        })}
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col space-y-4">
                  ...
                  <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-2xs">
                    <div className="p-3 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-slate-700">
                      <div className="flex items-center gap-2">
                        <span>ตารางประวัติผู้เข้าใช้งานระบบล่าสุด ({userAccessLogs.length} รายการ)</span>
                        <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-blue-50 text-blue-700 border border-blue-200">
                          ซิงค์อัตโนมัติ Real-Time
                        </span>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <button
                          onClick={() => {
                            const timeStr = new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' (ผ่าน LINE)';
                            const newTestLog: UserAccessLog = {
                              id: 'SESS-' + Date.now(),
                              time: timeStr,
                              userName: user?.displayName || 'ผู้ใช้งานจาก LINE Application',
                              email: user?.email || 'line.user@gmail.com',
                              role: isVhvUser ? 'อสม. (LINE App)' : 'ผู้ใช้งานผ่าน LINE',
                              ip: '182.52.231.' + Math.floor(Math.random() * 100),
                              location: '📍 ต.ไผ่ต่ำ อ.วิหารแดง จ.สระบุรี (14.3218, 100.9842)',
                              device: 'LINE App (iOS In-App Browser)',
                              status: 'ONLINE',
                              isMobile: true
                            };
                            setUserAccessLogs(prev => [newTestLog, ...prev]);
                          }}
                          className="px-2.5 py-1 bg-green-600 hover:bg-green-700 text-white text-[10px] font-bold rounded shadow-xs transition-colors cursor-pointer flex items-center gap-1"
                        >
                          <Smartphone className="w-3 h-3 text-green-200" />
                          <span>+ จำลองเข้าผ่าน LINE</span>
                        </button>

                        <button
                          onClick={() => {
                            const timeStr = new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' (จำลองมือถือ)';
                            const newTestLog: UserAccessLog = {
                              id: 'SESS-' + Date.now(),
                              time: timeStr,
                              userName: user?.displayName || 'ผู้ใช้งานโทรศัพท์มือถือ (Mobile User)',
                              email: user?.email || 'mobile.user@gmail.com',
                              role: 'ผู้ใช้งานมือถือ (Mobile)',
                              ip: '182.52.231.' + Math.floor(Math.random() * 100),
                              location: '📍 ต.ไผ่ต่ำ อ.วิหารแดง จ.สระบุรี (14.3218, 100.9842)',
                              device: 'Apple Safari / Chrome (iPhone Mobile)',
                              status: 'ONLINE',
                          ...
                          className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold rounded shadow-xs transition-colors cursor-pointer flex items-center gap-1"
                        >
                          <Smartphone className="w-3 h-3" />
                          <span>+ จำลองเข้าผ่านมือถือ</span>
                        </button>
                            ...
                            return (
                              <tr 
                                ...
                                className={`transition-colors ${
                                  isOnline ? 'bg-emerald-50/40 hover:bg-emerald-50' : 'hover:bg-slate-50'
                                }`}
                              >
                                ...
                                <td className="p-3 font-sans font-bold text-slate-900">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {log.isMobile && (
                                      <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[9px] font-bold bg-purple-50 text-purple-700 border border-purple-200" title="เข้าใช้งานผ่านโทรศัพท์มือถือ">
                                        <Smartphone className="w-2.5 h-2.5" />
                                        <span>มือถือ</span>
                                      </span>
                                    )}
                                  </div>
                                  <span className="block text-[10px] font-mono text-slate-500 font-normal">{log.email}</span>
                                </td>
                                <td className="p-3 font-sans">
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                    log.role.includes('เจ้าหน้าที่') || log.role.includes('Staff')
                                      ? 'bg-emerald-100 text-emerald-800 border border-emerald-200'
                                      : log.role.includes('อสม')
                                ...
                                <td className="p-3 text-slate-600">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className={log.isMobile ? 'font-bold text-purple-900' : ''}>{log.device}</span>
                                  </div>
                                </td>
